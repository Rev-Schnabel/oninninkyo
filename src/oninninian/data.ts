import { People } from "./types";

export const people: People[] = [
  {
    name: "ディオニュソス牧師 ◆wagQ2hhjjo",
    face: "（　＾ω＾）",
    apprenticeshipDate: "2012/01/27 22:39:46",
    apprenticeshipDateUrl: "https://uni.5ch.net/test/read.cgi/psy/1325761010/",
    pastorInaugurationDate: "2012/02/12 04:39:13",
    pastorInaugurationDateUrl: "https://uni.5ch.net/test/read.cgi/psy/1328889186/",
    description: "VIP にまで布教の為に出張するなどおにんにん教過激派の一面も覗かせるが、その信仰心たるや他の信者の追従を許さない。弟子の中では一番の古株であり、日々の努力が実った結果おにんにん総帥から「牧師」を名乗ることを許された存在。おにんにん教原理主義最先鋒である。氏の誕生日の 08/11 はおにんにん教の祝日の一つとなっており生誕祭として祝われている。残念ながら 2012/12/01 19:38:39 を最後に、2022 年 10 月現在、行方不明。第十三課イグニスが行方を捜索中。",
  },
  {
    name: "イグニス牧師 ◆cA7oIM8fok",
    face: "（　＾ω＾）",
    apprenticeshipDate: "2012/01/27 23:05:59",
    apprenticeshipDateUrl: "https://uni.5ch.net/test/read.cgi/psy/1325761010/",
    pastorInaugurationDate: "2012/12/30 10:31:44",
    pastorInaugurationDateUrl: "https://toro.5ch.net/test/read.cgi/siberia/1356692122",
    description: "以前に非行を行い、おにんにん総帥から破門を言い渡された。その後、は瀬川となるまでに落ちぶれたが、再び改心し祈りを捧げ続けた結果、おにんにん総帥から「弟子たちの多数決により元の姿に戻っても良い」との多大なる慈悲を受けることになり、無事元の姿に戻ることが出来た。その後研鑽を積み、2013 年より牧師となることを認められた。現在はおにんにん総帥からの信頼厚く、右腕として活躍している。",
  },
  {
    name: "ノトス ◆a/ipWNjqEOyZ",
    face: "（　＾ω＾）",
    apprenticeshipDate: "2012/01/29 20:04:21",
    apprenticeshipDateUrl: "https://uni.5ch.net/test/read.cgi/psy/1327750781/",
    pastorInaugurationDate: "",
    pastorInaugurationDateUrl: "",
    description: "真面目な信徒に思えたが、突如神殿を銃火器で急襲し、その後 2012/03/23 19:05:33 にﾐﾝｷｸへ堕ち、03/24 02:14:49 に破門となった。しかし、初期のノトスはトリップを付けていなかったため、破門されたノトスはノトスを乗っ取った魑魅魍魎であった可能性もある。",
  },
  {
    name: "エンシス ◆Ensis4mjh2",
    face: "（　＾ω＾）",
    apprenticeshipDate: "2012/01/29 20:11:20",
    apprenticeshipDateUrl: "https://uni.5ch.net/test/read.cgi/psy/1327750781/",
    pastorInaugurationDate: "",
    pastorInaugurationDateUrl: "",
    description: "沈着冷静で、頼りになる存在。おにんにん総帥の賛美歌を暗記しているらしい。2022 年 10 月現在、行方不明。",
  },
  {
    name: "シュナーベル牧師 ◆C/jyg/cVzU / ◆rZ7bAbN1dI",
    face: "（　＾ω＾）",
    apprenticeshipDate: "2012/02/04 01:16:07",
    apprenticeshipDateUrl: "https://uni.5ch.net/test/read.cgi/psy/1327750781/",
    pastorInaugurationDate: "2012/08/14 01:14:41",
    pastorInaugurationDateUrl: "https://toro.5ch.net/test/read.cgi/siberia/1344859490/",
    description: "多くは語らず黙って静かに祈りを捧げている姿をたびたび目撃されている。彼の寡黙で堂々たる祈りの姿は見る者を圧倒させる優美なものである。おにんにん総帥の教えを噛み砕きながら他の信徒へ説明する等の地道な積み重ねが認められ 2012/08/14 01:14:41 に牧師となった。",
  },
  {
    name: "シャマール ◆IctVvvPOHs",
    face: "（　＾ω＾）",
    apprenticeshipDate: "2012/02/04 21:51:39",
    apprenticeshipDateUrl: "https://uni.5ch.net/test/read.cgi/psy/1327750781/",
    pastorInaugurationDate: "",
    pastorInaugurationDateUrl: "",
    description: "温厚で慈悲深く、謙虚な姿勢を持つおにんにん総帥の愛弟子。アーダーを庇うなど面倒見の良さも彼の特徴である。2022 年 10 月現在、行方不明。",
  },
  {
    name: "ミレイユ ◆tyTXROdtXc",
    face: "ξ　＾ω＾ξ",
    apprenticeshipDate: "2012/02/05 17:39:48",
    apprenticeshipDateUrl: "https://uni.5ch.net/test/read.cgi/psy/1327750781/",
    pastorInaugurationDate: "",
    pastorInaugurationDateUrl: "",
    description: "お嬢様口調、上品なレスが特徴の紅一点。良家の生まれらしい。コーヒーより紅茶派。特にアールグレイを好んでいる。",
  },
  {
    name: "エイレーネ",
    face: "（　＾ω＾）",
    apprenticeshipDate: "2012/04/19 02:42:22",
    apprenticeshipDateUrl: "https://toro.5ch.net/test/read.cgi/siberia/1333020386/",
    pastorInaugurationDate: "",
    pastorInaugurationDateUrl: "",
    description: "自称ヒーニアス林檎という名の信者であったが、神殿での熱心な姿が認められ、おにんにん総帥からエイレーネという名を与えられた。2022 年 10 月現在、行方不明。",
  },
  {
    name: "デュナミス ◆WOXoeYTlQo",
    face: "（　＾ω＾）",
    apprenticeshipDate: "2012/07/16 20:50:25",
    apprenticeshipDateUrl: "https://toro.5ch.net/test/read.cgi/siberia/1341138609/",
    pastorInaugurationDate: "",
    pastorInaugurationDateUrl: "",
    description: "篤い信仰心を持つ信徒で、常に皆の健康を気にする心やさしき人物であったが、2022 年 10 月現在、行方不明。",
  },
  {
    name: "ラエティティア ◆eSdnZiwB6A",
    face: "（　＾ω＾）",
    apprenticeshipDate: "2012/07/20 02:23:10",
    apprenticeshipDateUrl: "https://toro.5ch.net/test/read.cgi/siberia/1341138609/",
    pastorInaugurationDate: "",
    pastorInaugurationDateUrl: "",
    description: "非常に柔らかい物腰で熱心に祈りを捧げる信徒。しかし、時折狂信的とも言える信仰を覗かせる。IT 関連の技術にも明るい。2022 年 10 月現在、行方不明。",
  },
  {
    name: "スプランクニゾマイ ◆Alice//LXs",
    face: "（　＾ω＾）",
    apprenticeshipDate: "2012/08/04 00:21:00",
    apprenticeshipDateUrl: "https://toro.5ch.net/test/read.cgi/siberia/1343208178/",
    pastorInaugurationDate: "",
    pastorInaugurationDateUrl: "",
    description: "自称カンヘルという名の信者であったが、イグニスが修行の旅に出ている間に神殿に活気をもたらし、その功績を認められおにんにん総帥からスプランクニゾマイという名を与えられた。2022 年 10 月現在、行方不明。",
  },
  {
    name: "ルーセント ◆7iXSkOsVgM",
    face: "（　＾ω＾）",
    apprenticeshipDate: "2012/08/11 22:19:36",
    apprenticeshipDateUrl: "https://toro.5ch.net/test/read.cgi/siberia/1343208178/",
    pastorInaugurationDate: "",
    pastorInaugurationDateUrl: "",
    description: "クリマと同時期に弟子入りした。クリマの暴走を受け、同期として共に罰を受けようとする、責任感の強い信徒。2022 年 10 月現在、行方不明。",
  },
  {
    name: "クリマ ◆nHEiDXi6lU",
    face: "（　＾ω＾）",
    apprenticeshipDate: "2012/08/11 22:20:47",
    apprenticeshipDateUrl: "https://toro.5ch.net/test/read.cgi/siberia/1343208178/",
    pastorInaugurationDate: "",
    pastorInaugurationDateUrl: "",
    description: "突然おにんにん教をやめると宣言し、2012/08/14 00:38:06 にわずか数日での実質破門となった。この騒動が弟子制度を見直すきっかけとなった。",
  },
  {
    name: "フェーリークス ◆.Apple.HfM",
    face: "（　＾ω＾）",
    apprenticeshipDate: "2012/09/17 18:39:54",
    apprenticeshipDateUrl: "https://toro.5ch.net/test/read.cgi/siberia/1346823121/",
    pastorInaugurationDate: "",
    pastorInaugurationDateUrl: "",
    description: "クゴウと名乗っていたが熱心に祈りを捧げ、牧師による「問掛の試煉」を突破し、弟子となった。積極的な神殿の建築等、精力的に活動していた。純粋な性格。純粋さゆえに、魑魅魍魎に振り回されることもあった。2022 年 10 月現在、行方不明。",
  },
  {
    name: "クレスケンスルーナ ◆jvdNOCQgZU",
    face: "（　＾ω＾）",
    apprenticeshipDate: "2013/03/02 00:40:16",
    apprenticeshipDateUrl: "https://toro.5ch.net/test/read.cgi/siberia/1359776314/",
    pastorInaugurationDate: "",
    pastorInaugurationDateUrl: "",
    description: "は瀬川としては初めて弟子となった。当初はは瀬川故に魑魅魍魎の疑いを持たれていたが、駄目元譲二を名乗り熱心に活動した結果、は瀬川なれど聖者の資質ありと認められ、弟子入りした。2022 年 10 月現在、行方不明。",
  },
  {
    name: "オルフェデルンボ　 ◆w1PVfgj8PY",
    face: "（　＾ω＾）",
    apprenticeshipDate: "2014/04/21 02:38:07",
    apprenticeshipDateUrl: "https://mastiff.5ch.net/test/read.cgi/siberia/1395630708/",
    pastorInaugurationDate: "",
    pastorInaugurationDateUrl: "",
    description: "自称オクレンボ田中として熱心に祈りを捧げ、弟子となった。自身のネットリテラシーの高さを活かし Wiki の運営方針やインターネット上での活動に対する知見を提供した。2022 年 10 月現在、行方不明。",
  },

];

export const otherPeople: People[] = [
  {
    name: "おにんにん教第13課イグニス / おにんにん教第十三課イグニス",
    face: "（　＾ω＾）",
    apprenticeshipDate: "",
    apprenticeshipDateUrl: "https://uni.5ch.net/test/read.cgi/psy/1327750781/",
    pastorInaugurationDate: "",
    pastorInaugurationDateUrl: "",
    description: "おにんにん教の中でも特に狂信的な集団。魑魅魍魎やその疑いがある者に過剰なまでの制裁を振るおうとし、おにんにん総帥に嗜められた。現在は諜報活動を主とし、イグニス牧師直属の組織として存在している模様。諜報活動の折、お土産やワインを購入しており、経費が膨れ上がっている。",
  },
  {
    name: "アスース",
    face: "（　＾ω＾）",
    apprenticeshipDate: "",
    apprenticeshipDateUrl: "https://toro.5ch.net/test/read.cgi/siberia/1349027051/",
    pastorInaugurationDate: "",
    pastorInaugurationDateUrl: "",
    description: "現在鍵垢となっているおにんにん教の Twitter アカウント管理者。おにんにん総帥や弟子達の名言を呟く bot を運用していた。",
  },
  {
    name: "ディオトドス",
    face: "（　＾ω＾）",
    apprenticeshipDate: "",
    apprenticeshipDateUrl: "https://toro.5ch.net/test/read.cgi/siberia/1349027051/",
    pastorInaugurationDate: "",
    pastorInaugurationDateUrl: "",
    description: "現在鍵垢となっているおにんにん教の Twitter アカウント副管理者。おにんにん総帥や弟子達の名言を呟く bot を運用していた。",
  },
  {
    name: "ヘルネスエス",
    face: "（　＾ω＾）",
    apprenticeshipDate: "",
    apprenticeshipDateUrl: "https://toro.5ch.net/test/read.cgi/siberia/1349027051/",
    pastorInaugurationDate: "",
    pastorInaugurationDateUrl: "",
    description: "時折、予言めいた詩のような文章を綴ってゆく人物。おにんにん総帥からの絶賛を受けた。",
  },
  {
    name: "おにんにん総帥の偽物 (優良なる偽物)",
    face: `
　　　 　 　／　　　　　　　　　　　　　　　　　　　　　＼
　　　 　 /　　,r'"j　　　　　　　　　　　　　　　 i^'!、　　ヽ
　　　　/　　 </´　　　　　　　　　　　　　　　　｀ヾ>　　.:;i,　
　　　 ,l　　　　　　　　_,._,.　　　　　　　 _,._,.　　　　　　 .:.:l,　
　　　 |　　　　　　　< (ヅ,>　　　　　 < (ヅ,>　　　　　...:.::|　
　　　 !　　　　　　 　｀￣´ 　 　 　. 　 ｀￣´ 　 　 　 ..: :::::!　
　　　｜　　　　　　　　　　 ﾉ . : . :;i,　　 　 　 　　 ... ::::::.:::|　
　 　　 !　　　　　　　　　　（.::.;人..;:::） 　　　　　...:.:::::.:::::::::!　
　　　　ヽ、　　　　　　　　　｀´　 ｀´　　　 ........::..::..::.::::::::/　
　　　　　 ＼......,,,,,,,_ 　　　　　　　　 　.....:::::::::::::::::::::::::／    
    `,
    apprenticeshipDate: "",
    apprenticeshipDateUrl: "https://toro.5ch.net/test/read.cgi/siberia/1349027051/",
    pastorInaugurationDate: "",
    pastorInaugurationDateUrl: "",
    description: "VIP に現れたおにんにん総帥の偽物。しかし、他の偽物と違い、気落ちしている >>1 に熱心に語りかけ、励ましており、偽物とは言えどもおにんにん総帥を唸らせた。",
  },
  {
    name: "フランチェスカ・ニコトロール",
    face: "（　＾ω＾）",
    apprenticeshipDate: "",
    apprenticeshipDateUrl: "",
    pastorInaugurationDate: "",
    pastorInaugurationDateUrl: "",
    description: "",
  },
  {
    name: "万歳",
    face: `
    　　　　 　 ,;-‐'ﾞ､^:^ｰ､.　　　　　　　　　　　　　　　　　　　　　　　 i' / ;' ;ﾞﾞヽ、
    　　　 　／ ､ r､.ﾞ:､ﾞi ヽ!　　　　　　　_,,,,,....-―--　､,,,_　　　　　　 　'ﾞﾞ、/_r 　ヽ、
    　　　　/　　iﾞｰ'^ｰ'‐'‐'　　 _,,.-‐:''""　　　　　　　　　　 ~\`'ｰ-.､　　　　　　｀i 　 ﾞ:､
    　　　 /　 　,;!　 　 　 　／　　　　　　　　　　　　　　　　　　　　　＼　　　 　|　 　ﾞ:
    　　. /　　 ,;: |　 　 　 /　　,r'"j　　　　　　　　　　　　　　　 i^'!、　　ヽ　　　 |　 　 ﾞi
    　　/　　　;;'_,!　　　./　　 <\´　　　　　　　　　　　　　　　　｀ヾ>　　.:;i,　 　_,!　　 　ﾞ:
    　.,ﾉｰ‐‐'''''" ﾞ7'ー ,l　　　　　　　　_,._,.　　　　　　　 _,._,.　　　　　　 .:.:l⌒\`i::ﾞヾ、___,,!
    　i'::::::ヾ;;::::::::　　　 |　　　　　　　< (●,>　　　　　 < (●,>　　 　　...:.::::|　　.:::::::::::::;;ﾉ　おにんにん総帥ばんざーいばんざーい
    　.ﾞヾ;;:::::ﾞ､::::;　　ミ !　　　　　　 　｀￣´ 　 　 　. 　 ｀￣´　　　　 ..: :::::::!　　.:::::::;;;／
    　　　｀ﾞﾞﾞ''ヾ;;!_　彡 |　 　　　　　　　　　 ﾉ . : . :;i,　　 　　　　　　.::::::::.::|　　-‐'"
    　　　　　　　　　　　!　　　　　　　　　　（.::.;人..;:::） 　　　　　.:::::::::::::::::::!￣"
    　　　　　　　　　　　ヽ、　　　　　　　　　\｀´　 \｀´　　　 ........::..::..::::::::::::/
    　　　　　　　　　 ＿　 ＼　　　　　　　　　　　　　　 ...:::::::::::::::::::::::::／ ＿＿
    　　　　　 　,. ''￣　　￣　＼,,,,,,,_ 　　　　　 　 　 ...:::::::::::::::::::::::::／'´￣　 　 ￣｀ヽ
    　　　　　　{、　　　　　　　　　　　\`ー-､＿＿＿＿-‐'"￣￣　 　　　　　　＿,ｨ　,{
    　　　　　　!　 / ﾄ､｀ｰ―'｀　　　　　　ﾉ　　　　　　丶　　　　　　　ｯｰ--‐'' イ､ 　 }
    　　　　　　{　;　 |　｀ｰ....,;_￣｀ー::;;;／　　　　　　　　＼　　.´ 　 _'__... -''´　{　l､　|
    　　　　　　|　; 　} 　　　　 ￣￣　　　　　　　　　　　　　￣￣￣　　　 　　　|　l　 |
    　　　　　　.!　　/　　　　　　　　　　　　　　　　　　　　　　　　　　　　　　　 ヾ　　|    
    `,
    apprenticeshipDate: "",
    apprenticeshipDateUrl: "",
    pastorInaugurationDate: "",
    pastorInaugurationDateUrl: "",
    description: "当初は誰から見ても邪悪で醜悪な魑魅魍魎に過ぎなかったが、自我がほとんど確認できないにも関わらずおにんにん総帥を崇拝している様子で、そのおにんにん総帥を慕う心根だけは本物であった。その姿に同情したおにんにん総帥から慈悲を与えられ徐々に人格が形成され始め、最終的には哲学者の様相を帯びるまでに至り、己の存在を問う旅に出た。魑魅魍魎から脱し、信徒として認められた数奇なる者である。",
  },
  {
    name: "草爺 / 草じぃ",
    face: "（　＾＾）ω",
    apprenticeshipDate: "",
    apprenticeshipDateUrl: "",
    pastorInaugurationDate: "",
    pastorInaugurationDateUrl: "",
    description: "",
  },
  {
    name: "ツンドリー ◆DoAsSEEYd.",
    face: "(ヾ'ω`)",
    apprenticeshipDate: "",
    apprenticeshipDateUrl: "",
    pastorInaugurationDate: "",
    pastorInaugurationDateUrl: "",
    description: "",
  },
  {
    name: "あにじゃ",
    face: `
　　　　 △　　　
　　　 （ ´｀ω） 　
　　　.ノ^　yヽ、　　
　　　ヽ,,ﾉ==l ﾉ 　　　
　　　　 ）ノ 　　　　　　
　　　 ▲ 　　　　　　　　
"""~""""""~"""~    
    `,
    apprenticeshipDate: "",
    apprenticeshipDateUrl: "https://toro.5ch.net/test/read.cgi/siberia/1373900023/",
    pastorInaugurationDate: "",
    pastorInaugurationDateUrl: "",
    description: "幽霊のような姿をしている。当初は挑戦的な物言いで反おにんにん教の魑魅魍魎と思われたが、時折鋭い意見を発する等、侮れない存在。",
  },
  {
    name: "ぽれ",
    face: "（　´ω｀）",
    apprenticeshipDate: "",
    apprenticeshipDateUrl: "",
    pastorInaugurationDate: "",
    pastorInaugurationDateUrl: "",
    description: "",
  },
];

export const evilSpirits: People[] = [
  {
    name: "フグリー",
    face: `
　　　　　　　　　＿＿＿＿＿
　　　 　 　 　 ／ﾉ　　　　　 ヽ ＼
　　　　　　／　／・＼　　／・＼　＼　　　
　　　　　 |　　　￣￣　　　￣￣　　 |
　　　　　 |　　　　（＿人＿）､ 　　　 |
　　　　　 |　　　　　＼　　　|　　　　 |
　 　 　 　 ＼　　　　　＼＿|　　　 ／
　　 　　　　　　　　|　|　　 | |
　　　　　　　　|　|　　 | | 　　　　
　　　　　 　　｜| 　　｜|
　　　　　　　　し|　 i　|J＝二フ
　　　　　　 　　 .|　 |｜
　　　　　　　 　｜ ﾉ ﾉ
　　　　　　　　　.| .| （
　　　　　　　　　/ |＼.＼　
　　　　　　　　　し' 　￣    
    `,
    apprenticeshipDate: "",
    apprenticeshipDateUrl: "https://uni.5ch.net/test/read.cgi/psy/1325761010/",
    pastorInaugurationDate: "",
    pastorInaugurationDateUrl: "",
    description: "かつての弟子の一人。おにんにん総帥の最初の弟子と言われている。自慰行為禁止という掟を二度破ったため破門となった。おにんにん総帥を逆恨みし神殿を荒らしていたところ、おにんにん総帥にフグリーであることを看破された。なおフグリーとはおにんにん総帥が与えた洗礼名ではなく、自ら名乗っていた。",
  },
  {
    name: "アバドン",
    face: `
　　　　　　　　　　　　　　　_i⌒)-､
　　　　　　　　　　　 ,ー-（　_,O ､.ﾉ--'-、
　.　 　 　 　 　 　 ／（　＾ω＾）廴（　＾ω＾＼
　.　 　 　 　 　 　/＾ω＾　　＾ω＾　＾ω　_,..,_',
　　　　　　　　　ﾉ　,r'で¨)ヾ,　　ｼ ,ｨで')ﾞ､__ }
　　　　　 　 　ﾉ　　 ￣￣´　 人　￣￣　 　',　　　
　　　　　　 　/　（　＾ω＾（　＾ω＾）（　＾ω＾）i　　　
　　　　　　　/＾ω＾　　　,.:::二二ﾆ:::.､.＾ω＾ i
　　　　　　ﾉ　　＾ω＾）　r'ｴ┴┴'ｰダ　　　,/
　　　　 ／　　＾ω＾）＾ωヾ=､＿_.／（　＾ω＾）　
　　　／　＾ω＾　＾ω＾）＾ω＾（　＾ω＾）（　＾ω＾）　　　　　　　　　　　　　
　　　(,,,,ﾉ（　｝(,,ﾉ(,,,ﾉ｛　）(,,,,ﾉ（　｝(,,ﾉ(,,,,ﾉ(,,,,ﾉ    
    `,
    apprenticeshipDate: "",
    apprenticeshipDateUrl: "https://uni.5ch.net/test/read.cgi/psy/1325761010/",
    pastorInaugurationDate: "",
    pastorInaugurationDateUrl: "",
    description: "禍々しい姿をした生物。かつておにんにん総帥の怒りに触れ、神の裁きを受けた反おにんにん教の成れ果て達。醜い姿を晒しながら永遠に苦しみ、生き続ける。永く生きたアバドンは時折知性を覗かせることがあり、そのような個体に対してはおにんにん総帥は祈りや神殿での一時の休息を許可する等、慈悲を与えることもある。中には数千年を生きたと思われる、「祈りという一つの現象そのもの」と化した個体もおり、おにんにん総帥とシュナーベル (当時) を驚愕させた。",
  },
  {
    name: "アーダー",
    face: "（　＾ω＾）",
    apprenticeshipDate: "",
    apprenticeshipDateUrl: "",
    pastorInaugurationDate: "",
    pastorInaugurationDateUrl: "",
    description: "弟子となることを希望しながらもミレイユに執拗なセクハラまがいの発言をする等、碌でもない人物。十三課イグニスによって即刻処断されかけるも、反省の様子を見せたためおにんにん総帥から慈悲を与えられた。しかし、再度問題行動を起こしおにんにん総帥の慈悲を仇で返した。愚物は決して矯正することがないという逸話として残されている。",
  },
  {
    name: "ナータランジャナム",
    face: "（　＾ω＾）",
    apprenticeshipDate: "",
    apprenticeshipDateUrl: "",
    pastorInaugurationDate: "",
    pastorInaugurationDateUrl: "",
    description: "",
  },
  {
    name: "オーウェン閣下 ◆CjxINalPTQ",
    face: "（　＾ω＾）",
    apprenticeshipDate: "",
    apprenticeshipDateUrl: "",
    pastorInaugurationDate: "",
    pastorInaugurationDateUrl: "",
    description: "",
  },
  {
    name: "タンザニア岡本",
    face: "（　＾ω＾）",
    apprenticeshipDate: "",
    apprenticeshipDateUrl: "",
    pastorInaugurationDate: "",
    pastorInaugurationDateUrl: "",
    description: "",
  },
  {
    name: "ソクラテス鬼塚",
    face: "┌(＾ ω＾)┘",
    apprenticeshipDate: "",
    apprenticeshipDateUrl: "",
    pastorInaugurationDate: "",
    pastorInaugurationDateUrl: "",
    description: "",
  },
  {
    name: "イマラーン",
    face: "（　＾ω＾）",
    apprenticeshipDate: "",
    apprenticeshipDateUrl: "",
    pastorInaugurationDate: "",
    pastorInaugurationDateUrl: "",
    description: "",
  },
  {
    name: "クロノス / スパークリングゾマホン",
    face: "（　＾ω＾）",
    apprenticeshipDate: "",
    apprenticeshipDateUrl: "",
    pastorInaugurationDate: "",
    pastorInaugurationDateUrl: "",
    description: "",
  },
];